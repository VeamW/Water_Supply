...
function selectInvoice(){
	showWindow({url:'<%=path %>/page/pay_window_selectInvoice.jsp'});
}
//发票补开
function reInvoice(){
	showWindow({url:'<%=path %>/page/pay_window_reInvoice.jsp'});
}
	...
	<div class="tab_content_container"> 
			...
			<table>
				...
				<tr>
					...
					<td colspan="4"><input size="16" />
						...
						<a href="<%=path %>/page/user_search1.jsp?id=0100000987" target="_blank" 
		...
		<div id="tab3" class="tab_content"> 
			...
			<table class="data display">
				...
				<tr class="odd">
					<td colspan="6">只显示最近的8条交费记录<%=path %>.</td>
				</tr>
				</tbody>
			</table>
		</div>

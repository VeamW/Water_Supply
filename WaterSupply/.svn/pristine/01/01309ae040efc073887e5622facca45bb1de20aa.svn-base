...
<head> 
	...
	<link rel="stylesheet" href="/Water_Project/css/reset.css" type="text//Water_Project/css" media="screen" title="no title" />
...
<div id="wrapper">
	...
	<div id="content" class="xgrid" >
		...
		<div class="x12">
			...
			<h2>快捷换表</h2>
			<form action="/Water_Project/usAction.do" method="post">
			...
			<table width="100%">
				...
				<form action="uumAction.do" method="post">
				<tbody>
				<c:if test="${i==1 }">
					查询用户的各项值可能不匹配，请谨慎检查！！
				</c:if>
					<tr>
						<td>用户编码</td>
						<td><input disabled="disabled" value="${us.userNo }" /></td>
						<td>用户姓名</td>
						<td><input disabled="disabled" value="${us.userName }" /></td>
						<td>原表类型</td>
						<td><input disabled="disabled" value="${smt.meterTypeName }" /></td>
					</tr>
					<tr>
					<input type="hidden" name="meterNo" value="${um.meterNo }" readonly="readonly"/>
					<input type="hidden" name="disabled" value="0" readonly="readonly"/>
					<input type="hidden" name="userNo" value="${um.userNo }" readonly="readonly"/>
						<td>新表编号</td>
						<td><input readonly="readonly" name="meterNo" disabled="disabled" value="自动生成" /></td>
						<td>表身码</td>
						<td><input name="meterName" value="${um.meterName }" /></td>
						<td>起始底码</td>
						<td><input name="startValue" value="${um.startValue }" /></td>
					</tr>
					<tr>
						<td>水表类型</td>
						<td>
						<select style="width:158px;" name="meterTypeId">
							...
							<c:forEach items="${listsmt}" var="s">
								<option value="${s.id}">${s.meterTypeName}</option>
							</c:forEach>
						</select></td>
						<td>水表口径</td>
						<td><input value="${smt.aperture }" readonly="readonly"/></td>
						<td>最大码值</td>
						<td><input name="maxValue" value="${um.maxValue }" onClick="WdatePicker();" /></td>
					</tr>
					<tr>
					
						<td>装表日期</td>
						<td><input name="setupDate" value="<fmt:formatDate value="${um.setupDate }" timeStyle="yyyy-MM-dd" />" /></td>
						<td>水表厂家</td>
						<td colspan="3"><input size="68" name="factory" value="${um.factory }" /></td>
					</tr>
				</tbody>
				</table>
				
				<div style=" margin:20px 0px 0px 66px;" >
					<input type="submit"  class="btn" value="更换水表"/>
				</div>
